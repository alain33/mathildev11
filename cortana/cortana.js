...
//filePathdata5 = path.resolve('%CD%', './mémoiredemathilde/memomemoire/memo.json').replace('\\%CD%', '');
...
if(nameList[j].length==nameListe[j].length){console.log('          '+nameList[j].length+'           '+nameListe[j].length)
  nomduplug=objet5.nompluguine[g]
    console.log(objet5.nompluguine[g]); emulate1(query,nomduplug);/////fin
...
console.log('pas de reco dans mes xml je test en direct')
...
setTimeout(function(){ 
 fs=require('fs')
filePathfichier = path.resolve('%CD%', './bin/'+ladate+'.log').replace('\\%CD%', '');

fichier=fs.readFileSync(filePathfichier,'utf-8')
//fichier=fs.readFileSync('C:/Users/Administrateur/sarah/bin/'+ladate+'.log','utf-8')

objet = fichier.toString()
jsonStr = JSON.stringify(objet);
//console.log(jsonStr)
function allIndexOf(str, toSearch,query) {lo=''
    var indices = [];
    for(var pos = str.indexOf(toSearch); pos !== -1; pos = str.indexOf(toSearch, pos + 1)) {
        indices.push(pos); for(i=0;i<100;i++){lo=lo+objet[pos+i]}
    }
    if(lo.indexOf('Build')>-1){console.log('trouver');return false};
   console.log(pos)
   if(pos == -1){Skynet(query)} 
    return indices;

//console.log(objet[pos+i])
}//fin fnct allIndexoff

//console.log(new Date())
 allIndexOf(objet,letemps,query)
; }, 3000);//fin timeout
}//fin fnct xmlinconnu
...
filePathcontent = path.resolve('%CD%', './custom.ini').replace('\\%CD%', '');
content = fs.readFileSync(filePathcontent,'utf8');
//content = fs.readFileSync("custom.ini",'utf8');
emplacement=content.search("name=")
contente20=""
for (i=0;i<20;i++){contente20=contente20+content[emplacement+5+i]}
...
request({ url : url1 }, function (err, response, body){
ScribeSpeak("Pluguine "+nomduplug);callback();return false
});//fin request
...
//var answer1 = 'Voici le raisultat de votre recherche sur ' + reponse ;
 if (reponse==""){reponse="false"}
 SARAH.run('meteomathilde', { 'ask' : reponse});callback({'tts' : " "})
  }
...
//var patt = new RegExp;
 ...
 if(reponse==0){console.log('vide');callback({'tts' : " "});return false}
...
ScribeSpeak("réveil programmé à "+reponse1)
...
if (nameList[j].search(new RegExp("\\b" + nameListe[j] + "\\b","gi"))>-1){
  ...
  if (j==longueur2-1){console.log('on as trouver') ;emulate1(reponse3);
...
if(match<0){reponse=reponse.toLowerCase();query=query.toLowerCase();match=query.search(new RegExp("\\b" + reponse + "\\b","gi"));console.log(reponse+query)}

if(match<0){reponse=reponse.toUpperCase();match=query.search(new RegExp("\\b" + reponse + "\\b","gi"));console.log(reponse)}
//.toLowerCase();
if(match>-1){
